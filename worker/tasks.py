...
def run_submission_in_sandbox(submission_id: int, problem_id: str, code: str,
    ...
    try:
        ...
        # Buscar metadata del problema usando ProblemService (búsqueda recursiva)
        try:
            problem_dir = problem_service.get_problem_dir(problem_id)
        except Exception as e:
            logger.error(
                "Problem directory not found",
                extra={"problem_id": problem_id, "error": str(e)}
            )
